...
export default {
  ...
  computed: {
    rawValue () {
      const { dataType, dataTypeProp, multiple, modelValue } = this
      if (dataType) {
        if (multiple) {
          return modelValue?.map(value => value instanceof dataType ? value[dataTypeProp] : value)
        } else {
      ...
      if (this.dataType) {
        ...
        if (this.multiple) {
          const typedValues = value.map(current => {
            ...
            return new DataType(current, option?.text)
          })
          this.$emit('update:modelValue', typedValues)
        } else {
          const option = this.options.find(option => option.value === value)
          const typedValue = new DataType(value, option?.text)
